...
static PyObject* apply_blend_normal(PyObject *args){
#define ALPHA_BLEND_COLOR_CHANNEL(color1, alpha1, color2, alpha2) BLEND_NORMAL(CALC_COLOR(color1, alpha1), CALC_COLOR(color2, alpha2) * (alpha1 / MAX_RGB_DOUBLE))
#include "_composite.def"
}

static PyObject* apply_blend_screen(PyObject *args){
#define ALPHA_BLEND_COLOR_CHANNEL(color1, alpha1, color2, alpha2) BLEND_SCREEN(CALC_COLOR(color1, alpha1), CALC_COLOR(color2, alpha2))
#include "_composite.def"
}
...
{
    PyObject *py_image1 = NULL, *py_image2 = NULL, *image_mode = NULL, *blend = NULL,
             *w1, *h1, *w2, *h2, *py_x, *py_y, *py_merge = NULL;

    if (!PyArg_UnpackTuple(args, "apply", 10, 11, &image_mode, &py_image1, &w1, &h1, &py_image2, &w2, &h2, &py_x, &py_y, &py_merge, &blend)) {
        return NULL;
    }

    int blend_mode = (int)PyInt_AsLong(blend);
    switch(blend_mode){
        case MODE_MULTIPLY:
            return apply_blend_multiply(args);
            break;
        case MODE_SCREEN:
            return apply_blend_screen(args);
            break;
        default:
            return apply_blend_normal(args);
            break;
    }

    //return apply_composite(args);
    /*
    ...
    for (y = start_y; y < height2; ++y) {
        ...
        for (x = start_x; x < width2; ++x, aux1 += num_bytes, aux2 += num_bytes) {
            ...
            if (merge) {
                ...
                r = delta * ALPHA_COMPOSITE_COLOR_CHANNEL3(r2, a2, r1, a1);
                g = delta * ALPHA_COMPOSITE_COLOR_CHANNEL3(g2, a2, g1, a1);
                b = delta * ALPHA_COMPOSITE_COLOR_CHANNEL3(b2, a2, b1, a1);
            } else {
                if (a1 == 0) {
                    r = r2;
                    g = g2;
...
FILTER_MODULE(_composite,
    "apply(image_mode, buffer1, width1, height1, buffer2, width2, height2, pos_x, pos_ys, merge, blend) -> string\n"
